...
class KMeans(object):
    ...
    def train(self, data, iters):
        ...
        while i < iters and not flag_groups and not flag_centroids:
            ...
            # data points.
            changed_groups = np.where(groups != cur_groups, 1, 0)
            flag_groups = (changed_groups == 0).all(axis=None)

            # TODO: Check if there are changes with the values of the centroids
            changed_centroids = (self.centroids - centroids)
            flag_centroids = (changed_centroids == 0).all(axis=None)
